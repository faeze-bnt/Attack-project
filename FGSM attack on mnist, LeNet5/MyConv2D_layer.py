...
from keras import activations as Ac
...
class MyConv2D(layers.Layer):
    ...
    def call(self, x):
        ...
        if self.Method == 'Float':     # Method 1 - convolution: using original keras bulit-in function
            ...
            output1 = N.conv2d(A_stream, B_stream, strides=1, padding='VALID')
            #-------------------------------------------------------------------------------
            output = Quantizer(output1/self.Stream_Length, self.Width)
        output = Ac.relu(output + self.bias)
    ...
    def predict(self, x):
        if self.Method == 'Float':     # Method 1 - convolution: using original keras bulit-in function
            output = N.conv2d(x, self.kernels, padding='VALID')       # Do covolution using keras bulit-in function with floating-point inputs
        elif self.Method == 'Fixed':                                             
            x = Quantizer(x, self.Width) 							      # Quantize Input
            kernels = Quantizer(self.kernels, self.Width)
            #self.kernels = Quantizer(self.kernels, self.Width)  	      # Quantize Kernels
            output1 = N.conv2d(x, kernels, padding='VALID')     	  # Do covolution using keras bulit-in function with Quantizer inputs
            output = Quantizer(output1, self.Width)
            #output = output1
        elif self.Method == 'Stoch':
            x = Quantizer(x, self.Width)
            ...
            #-------------------------------------------------------------------------------
            output = Quantizer(output1/self.Stream_Length, self.Width)
        output = N.relu(output + self.bias)
        return output
