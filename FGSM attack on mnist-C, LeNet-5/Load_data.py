...
class Load_DATA:
    def __init__(self, error=False, train_samples=60000, test_samples=10000, validation_samples=5000, 
        ...
        # data_dir = "mnist_c_data"
        # url = "https://zenodo.org/record/3239543/files/mnist_c.zip"
        # if not os.path.exists(data_dir):
        #     self._download_and_extract(url, data_dir)
            
            # os.system("wget https://zenodo.org/record/3239543/files/mnist_c.zip && unzip mnist_c.zip -d mnist_c_data && rm mnist_c.zip")
        # Load one corruption type, e.g., 'motion_blur' (can be customized)
        corruption = "motion_blur"
        x_train = np.load(f"{corruption}/train_images.npy")
        y_train = np.load(f"{corruption}/train_labels.npy")
        x_test = np.load(f"{corruption}/test_images.npy")
        y_test = np.load(f"{corruption}/test_labels.npy")
