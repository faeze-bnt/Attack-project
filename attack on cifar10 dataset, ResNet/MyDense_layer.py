...
class MyDense(layers.Layer):
  ...
  def call(self, x):
    ...
    if self.Method == 'Float':     # Method 1 - convolution: using original keras bulit-in function
        ...
        x = Quantizer(x, self.WidthIn, 'floor', clip = True); 							   # Quantize Input
        self.kernel = Quantizer(self.kernel, self.WidthIn, 'round', clip = True);  	       # Quantize Kernel
        output = y = tf.matmul(x, self.kernel)    # Do multiplication using keras bulit-in function with Quantizer inputs and output
        ...
        B_stream = Stochastic_SNG_B(self.kernel, self.Str_Len, self.SNG_num[1]);
