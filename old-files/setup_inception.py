...
class NodeLookup(object):
  ...
  def load(self, label_lookup_path, uid_lookup_path):
    ...
    """
    if not tf.compat.v1.gfile.Exists(uid_lookup_path):
      tf.logging.fatal('File does not exist %s', uid_lookup_path)
    if not tf.compat.v1.gfile.Exists(label_lookup_path):
...
def run_inference_on_image(image):
  ...
  with tf.compat.v1.Session() as sess:
    ...
    #dat = transform.resize(image, (299,299))
    #dat = scipy.misc.imresize(scipy.misc.imread(image),(299,299))
    #dat = np.array((iio.imread(image)).resize((299,299)))
    image_read = Image.open(image)
    dat = zoom(np.array(image_read), (299 / image_read.size[0], 299 / image_read.size[1], 1))
