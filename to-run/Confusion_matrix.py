...
class Conf_matrix:
  def __init__(self, trues, predictions):
    true_label = self.one_hot_to_binary(trues)
    pred_label = self.one_hot_to_binary(predictions)
    ...
    fig, ax = plt.subplots()
    sns.heatmap(c_matrix, annot=True, ax=ax, cmap='Blues')  # Annotate the cells with the numeric values
